...
module.exports.parseSampleTableBox = function(payload, parentInfo) {
    var offset = 0;
    var kv_array = [
    ...
    switch (parentInfo['handlerType']) {
        case 'soun':
            kv_array = kv_array.concat([
                {key: "Channel Count", value: payload.readUIntBE(offset+8, 2)},
                {key: "Sample Size", value: payload.readUIntBE(offset+10, 2)},
                {key: "FourCC", value: payload.slice(offset+12, offset+16).toString('ascii')},
                {key: "Sample Rate", value: payload.readUIntBE(offset+16, 4)},
            ]);
            break;
        case 'vide':
            kv_array = kv_array.concat([
                {key: "FourCC", value: payload.slice(offset+12, offset+16).toString('ascii')},
                {key: "Width", value: payload.readIntBE(offset+16, 2)},
                {key: "Height", value: payload.readIntBE(offset+18, 2)},
                {key: "Horizresolution", value: payload.readUIntBE(offset+20, 4)},
                {key: "Vertresolution", value: payload.readUIntBE(offset+24, 4)},
                /* reserved +4 */
                {key: "Frame Count", value: payload.readUIntBE(offset+32, 2)},
                {key: "Compressor Name", value: payload.slice(offset+34, offset+66).toString('ascii')},
                {key: "Depth", value: payload.readUIntBE(offset+66, 2)}
            ]);
            break;
        default: //case 'hint':
            kv_array = kv_array.concat([
                {key: "Data", value: payload.readIntBE(offset, 1)},
                {key: "FourCC", value: payload.slice(offset+12, offset+16).toString('ascii')}
            ]);
            break;
    }
...
module.exports.parseSampleSizeBox = function(payload) {
    const fullbox = getFullBox(payload);
    let offset = fullbox.size;
    ...
    if (sampleSize === 0) {
        for (let i=0; i<sampleCount; i++) {
            kv_array = kv_array.concat([
                {key: "Entry Size", value: payload.readUIntBE(offset+8+(4*i), 4)}
            ]);
        }
    }
...
module.exports.parseChunkOffsetBox = function(payload) {
    const fullbox = getFullBox(payload);
    let offset = fullbox.size;
    ...
    for (let i=0; i<entryCount; i++) {
        kv_array = kv_array.concat([
            {key: "Chunk Offset", value: payload.readUIntBE(offset+4+(4*i), 4)}
        ]);
    }
