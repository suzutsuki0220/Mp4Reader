...
function setParentInfo(atom) {
    if (atom.type === 'hdlr') {
        ...
        const handler = kv.find(k => k.key === 'Handler Type');
        atom.parentInfo['handlerType'] = handler.value;
    } else if (atom.type === 'stsz') {
        const kv = parser.parseSampleSizeBox(atom.payload);
        const sizes = kv.filter(k => k.key === 'Entry Size');
        atom.parentInfo['entrySize'] = sizes.map(v => v.value);
    } else if (atom.type === 'stco') {
        const kv = parser.parseChunkOffsetBox(atom.payload);
        const offsets = kv.filter(k => k.key === 'Chunk Offset');
        atom.parentInfo['chunkOffset'] = offsets.map(v =>v.value);
    }
}
